...
def train_model(df: pd.DataFrame):
    ...
    # Заменяем простые NaN на медиану (GradientBoosting не умеет в NaN)
    X = X.fillna(X.median())
    ...
    # GradientBoosting стабильнее HistGradientBoosting на малых выборках
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        # Жёсткая обрезка в физиологичные границы [0, 100]
...
def main():
    ...
    output_dir = project_root / "ml_outputs_v3_stable"
    ...
    # Сохраняем в ТОЧНО такой же путь, который ждёт твой скрипт оценки
