...
def train_model(df: pd.DataFrame):
    ...
    X["deep_node"] = X["depth"] / (X["depth"].max() + 1e-6)
    model_features = FEATURE_COLUMNS + ["bl_cv", "gap_var_clade", "gc_extreme", "tiny_clade", "deep_node"]
    X = X[model_features]
    ...
    pipe = Pipeline([
        ...
        ("rf", RandomForestRegressor(
            random_state=42,
            n_jobs=-1,
            criterion="squared_error",  # прямая оптимизация под R^2
            bootstrap=True
        ))
    ])
    ...
    param_dist = {
        ...
        'rf__min_samples_leaf': randint(3, 15),
    ...
    print("[INFO] Запуск подбора гиперпараметров для Random Forest...")
    ...
    final_fold_idx = list(gkf.split(X, y, groups))[-1][1]
    X_test, y_test, g_test = X.iloc[final_fold_idx], y.iloc[final_fold_idx], groups.iloc[final_fold_idx]
    y_pred = np.clip(best_model.predict(X_test), 0.0, 100.0)
    ...
    metrics = {
        ...
        "pred_range": f"{y_pred.min():.1f}–{y_pred.max():.1f}",
    ...
    pred_df = pd.DataFrame({
        ...
        "y_pred": y_pred,
        "dataset_id": g_test.values
    })

    return best_model, metrics, feature_importance, pred_df
